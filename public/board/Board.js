...
export default class Board {
    ...
    checkWinner(column) {
        ...
        let maxRow = Math.min(this.#lastDrop.row + 3, this.#columns[0].rows.length - 1);
        ...
        for (let i = minCol; i <= maxCol - 3; i++) {
            playerCount = 0;
            for (let j = 0; j <= 3; j++) {
                if (this.#columns[i + j].rows[this.#lastDrop.row] === this.#columns[this.#lastDrop.column].rows[this.#lastDrop.row])
                    playerCount++;
            }
            if (playerCount >= 4)
        ...
        for (let i = minRow; i <= maxRow - 3; i++) {
            playerCount = 0;
            for (let j = 0; j <= 3; j++) {
                if (this.#columns[this.#lastDrop.column].rows[i + j] === this.#columns[this.#lastDrop.column].rows[this.#lastDrop.row])
                    playerCount++;
            }
            if (playerCount >= 4)
                return this.#columns[this.#lastDrop.column].rows[this.#lastDrop.row];
        ...
        // Forward Slash
        let currentRow = (this.#lastDrop.row - Math.min(this.#lastDrop.row - minRow, this.#lastDrop.column - minCol));
        let currentCol = (this.#lastDrop.column - Math.min(this.#lastDrop.row - minRow, this.#lastDrop.column - minCol));
        if (currentCol <= 3 && currentRow <= 2) {
            while (currentRow <= maxRow - 3 && currentCol <= maxCol - 3) {
                playerCount = 0;
                for (let i = 0; i <= 3; i++) {
                    if (this.#columns[currentCol + i].rows[currentRow + i] === this.#columns[this.#lastDrop.column].rows[this.#lastDrop.row])
                        playerCount++;
                }
                if (playerCount >= 4)
                    return this.#columns[this.#lastDrop.column].rows[this.#lastDrop.row];
        ...
        // Backwards Slash
        currentRow = (this.#lastDrop.row - Math.min(this.#lastDrop.row - minRow, maxCol - this.#lastDrop.column));
        currentCol = (this.#lastDrop.column + Math.min(this.#lastDrop.row - minRow, maxCol - this.#lastDrop.column));
        if (currentCol >= 3 && currentRow <= 2) {
